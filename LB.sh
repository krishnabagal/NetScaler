...
echo "Enter 4 to search host present in ServiceGrp"
echo ""
...
do
  ...
  hosts=$(cat LBConfig.txt |grep -i "bind\ serviceGroup" |grep -E "(^| )$sergrp( |$)" |grep -v monitorName |awk {'print $4":"$5'})
    ...
    VS=$(/usr/local/nagios/libexec/check_netscaler_vserver.pl -H xx.xx.xx.xx -C icinga -S $V)
...
list1=$(cat LBConfig.txt |grep -i "bind\ serviceGroup"  |grep -i $Host | awk {'print $3'} |sed "s|$|:|g"|tr -d "\n" |tr -s ":" " ")

for a in $list1
do
echo "$a:"
cat LBConfig.txt |grep -i "bind\ serviceGroup" |grep -E "(^| )$a( |$)" |grep -v monitorName |awk {'print $4":"$5'}
echo --------
done
